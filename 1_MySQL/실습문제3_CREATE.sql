...
CREATE TABLE book(
    ...
    bk_autor VARCHAR(20) NOT NULL,
    bk_price INT,
	bk_pub_no INT,
    CONSTRAINT FOREIGN KEY (bk_pub_no) 
		REFERENCES publisher(pub_no) ON DELETE CASCADE
);


INSERT INTO book(bk_title, bk_author, bk_price, bk_pub_no)
VALUES('개발자를 위한 생각의 정리 문서작성법', '카이마이 마즈히로', 20000, 1);
INSERT INTO book(bk_title, bk_author, bk_price, bk_pub_no)
...
SELECT member_name, member_id, rent_date, adddate(rent_date, INTERVAL 7 DAY) "반납 예정일"
FROM rent
...
SELECT bk_title, pub_name, rent_date, adddate(rent_date, INTERVAL 7 DAY) "반납 예정일"
FROM rent
