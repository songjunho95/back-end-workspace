...
CREATE PROCEDURE dell_all_emp(IN id INT)
BEGIN
	DELETE FROM emp_copy
    WHERE emp_id = id;
END //
...
/*
    ...
    DELIMITER ;
*/
-- 사번을 입력받아 해당 사원의 연봉을 게산하고 리턴하는 함수 생성
DELIMITER //
...
/*
    ...
    - 입출고에 대한 데이터가 기록(INSERT) 될 때마다 해당 상품에 대한 재고수량을 배번 수정(UPDATE) 해야 되는 경우 
...
-- employee 테이블에 새로운 행이 INSERT 될 때마다 '신입사원이 입사했습니다' 라는 메시지를 log 테이블에 자동으로 INSERT 되는 트리거 생성
