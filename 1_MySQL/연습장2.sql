...
/*
    ...
    - 서브쿼리를 수행하 ㄴ결과값이 몇 행 몇 열이냐에 따라 분류
    - 서브쿼리 종류에 따라 서브쿼리와 사용하는 연산자가 달라짐
    
    1. 단일행 서브쿼리 (SINGLE ROW SUBQUERY)
    - 서브쿼리의 조회 결과값의 개수가 오로지 1개일 떄 (한 행 한 열)
    - 일반 비교연산자 사용 가능 : =. !=, <>, >, <, >=, <=, ...
...
-- 2) 3047662.6087 보다 더 많은 급여를 받는 사원들의 사번, 사원명, 직급코드, 급여 조회
...
-- 전지연 사원이 속해있는 부서원들의 사번, 직원며으 전화번호, 직급명, 부서명, 입사일 조회
-- 단, 전지연 사원은 제외
...
-- 2) 부서코드가 D1인 직원들의 사번, 직원며으 전화번호, 직급명, 부서명, 입사일 조회
...
/*
	2. 다중행 서브쿼리
    - 서브쿼리의 조회 결과 값의 개수가 여러 행 일 떄 (여러행 한열)
...
SELECT max(salary)
FROM employee
GROUP BY dept_code; -- 2890000, 3660000

-- 2) 위의 값들을 받는 직원의 이름, 직급코드, 부서코드, 급여 조회
SELECT emp_name, job_code, dept_code, salary
FROM employee
WHERE salary IN (28900000, 3660000, 2490000, 3760000, 3900000, 2550000, 8000000);
...
-- 직원에 대한 사버느 이름, 부서코드, 구분(사수/사원) 조회
...
*/
-- 과장 지급임에도 차장 직급의 최대 급여보다 더 많이 받는 사원들의 사번, 이름, 직급, 급여 조회
-- 차장 직급의 급여들
SELECT salary
FROM employee
JOIN job USING (job_code)
...
FROM employee
WHERE dept_code = (SELECT job_code
					FROM employee
					 WHERE emp_name = '하이유');
