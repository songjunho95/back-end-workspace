...
-- 2) D1 부서원들의 사번, 직원명, 전화번호, 직급명, 부서명, 입사일 조회
...
*/
-- 각 부서별 최고 급여를 받는 직원의 이름, 지급코드, 부서코드, 급여 조회
-- 1) 각 부서별 최고 급여

SELECT max(salary)
FROM employee
GROUP BY dept_code; -- 2890000, 3660000, 2490000, 3760000, 
					-- 3900000, 2550000, 80000
...
SELECT *
FROM employee; -- emp_id, emp_name, dept_code, manager_id
...
FROM employee
JOIN employee USING manager_id) 
WHERE manager_id = (SELECT "사수", emp_id, emp_name, dept_code
					FROM employee);
