...
2번
- GROUP_BY&HAVING 문제 나옵니다.
- 2문제 나옵니다.


마지막
- JOIN관련된 문제 나옵니다.
- 이너 조인만 문제 나옵니다. (에프터 조인 문제 안나옵니다.)
- 셀렉트 펑션 초반 부분 집중해서 보기 함수는 코드에서 거의 제공됨
- 
...
/*
  ...
  - SQL 종류
	...
	5. TCL(Transaction Control Language) : 트랜잭셔을 제어하는 언어
...
FROM employee;
-- 5. emplyee 테이블의 입사일(hire_date), 직원명(emp_name), 급여(salary) 조회
...
/*
   ...
   - 조회하고자 하는 테이블로부터 특정 조건에 만족하는 데이터만 조화하고자 할 때 사용 
   - 이때 WHERE 절에 조건식을 제시 
   - 조건식에는 다양한 연산자 사용 가능
...
SELECT ent_yn, emp_id, emp_name, hire_date
FROM employee
WHERE ent_yn = 'N';
...
/*
  ...
  비교대상컬럼 LIKE '%문자%' => 비겨대상컬럼값이 문자가 "포함"되는걸 조회 (키워드 검색)
...
ORDER BY 1 bonus DESC; -- 보너스 기준 내림차순 정렬 (null이 맨뒤

/*
  LIMIT
  - ORDER BY 절 보다 뒤에 조건을 걸고 싶을 떄 사용
  - 출력되는 행 수를 제한하는 MySQL 전용 비표준 구문
  - 데이터 양을 제한하고자 할 떄 유용 (페이징처리)
...
-- LIMIT 절은 두개의 값이 있을 수 있음!
-- 첫번째 값은 오프셋(offset, 0부터시작) 시작 행을 지정
...
-- 3. 연봉(보너스포함X)이 4000 만원 이상이고 보너스를 받지 않은 사원들의 사번(emp_id), 사원명, 급여, 보너스(bonus) 조회

-- 4. 입사일이 '1995-01-01' 이상이고 부서배치를 받은 사원들의 사번, 사원며으 입사일(hire_date), 부서코드 조회(정렬은 먼저 입사한 기준)
...
-- 6. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스 포함 연봉(별칭부여) 조회 (연봉이 높은 순으로 정렬) , 보너스 포함 연봉 : (SALARY + SALARY * BONUS) * 12
