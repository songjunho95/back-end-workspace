/*
   JOIN
   - 두 개 이상의 테이블에서 데이터를 조회학자 할 때 사용하는 구문
...
-- 2) 연결할 두 컬럼명이 같은 경우 (employee : job_code - job : jon_code)
-- 사번, 사원명, 직급코드, 직급명 조회 
SELECT emp_id, emp_name, dept_code, job_name
FROM employee, job
WHERE job_code = job_code; -- amibiguous : 애매한, 모호한 / 에러 발생!

-- 해결방법 1) 테이블에 별칭 부여해서 이용
...
-- 자연조인(NATURAL JOIN) : 각 테이블마다 동일ㄹ한 컬럼이 한 개만 존재할 경우
-- 주의사항! 쓰지마세요...!! 
SELECT emp_id, emp_name, job_code, job_name
FROM employee
NATURAL JOIN job;

-- 직급인 대리인 사원의 사번(emp_id), 이름(emp_name), 직급명(job_name), 급여(salary) 조회
...
-- 실습문제 --
-- 1. 부서가 인사관리부인 사원들의 사번, 이름, 보너스 조회(employeee, department)
...
-- 3. 보너스를 받는 사원들의 사번, 사원명, 보너스, 부서명 조회(employeem, department)
...
-- 4. 부서가 총무부가 아닌 사원들의 사원명, 급여 조회(emplpyee, department)
...
/*
    2. 외부조인(OUTER JOIN) : MYSQL은 ANSI 구문만 가능
...
SELECT e.emp_id "사원사번", e.emp_name "사원명", e.dept_code "사원부서코드", e.manager_id "사수사번", m.emp_id "사수사번", m.emp_name "사수명", m.dept_code "사수부서코드"
FROM employee e
LEFT JOIN employee m ON (m.emp_id = e.manager_id);

/*
    5. 카테시안곱 (CARTESIAN PRODUCT) / 교차 조인 (CROSS JOIN)
      - 조인되는 모든 테이블의 각 행들이 서로사로 모두 매핑된 데이터가 검색된다. (곱집합)
...
-- 실습문제 --- 
-- 1. 직급이 대리면서 ASIA 지역에서 근무하는 직원들의 사번, 직원명, 직급명, 근무지역, 급여 조회


SELECT job_name, emp_id, emp_name, dept_code, local_name, salary  -- job_name, local_nae
FROM employee
	JOIN department ON (dept_code = dept_id)
	JOIN job USING (job_code)
    JOIN location ON (location_id = local_code)
...
SELECT * FROM location; -- local code
...
--    부서가 없는 직원들도 나태내고 싶다면 LEFT JOIN (employee 테이블인 왼쪽에 있을 떄)
...
SELECT 
    dept_title, format(avg(salary), 0)
FROM employee
	JOIN department ON (dept_code = dept_id)
GROUP BY dept_title;
...
if(sal_level IN ('S1', 'S2'), '고급', 
    ...
    CASE WHEN sal_level IN ('S11', 'S2') THEN '고급'
...
SELECT emp_id, emp_name, sal_levelm
...
-- 9. 부서가 있는 직원들의 직우너며으 직급명, 부서명, 근무지역 조회
-- INNER JOIN을 하는 경우 null 이 포함되어 있는 것처럼 일치하는 걸 못찾는다면 제외
...
-- 12. 사번, 사원명, 부서명, 직급명, 지역명, 국가명, 급여등급 조회 참고로 employee, job, department, location, national, sal_grade

SELECT * FROM employee; -- emp_id, emp_name, dept_code, job_code
SELECT * FROM department; -- dept_id, dept_title, location_id
SELECT * FROM job; -- job_code, job_name
SELECT * FROM locatioon; -- local_code, local_name, national_code
SELECT * FROM national; -- national_code, national_name
SELECT * FROM sal_grade; -- sal_level, min_sal, max_sal
