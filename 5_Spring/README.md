...
1. DI(Dependency Injection, 의존성 주입) : 설정 파일이나 어노테이션ㅇ을 통해 객체간의 의존 관계를 설정하여 개발자가 직접 객체 간의 의존 관계를 생성할 필요가 없다.
...
- 프로그램을 구동하는데 필ㅇ한 객체에 대한 생성, 변경 등의 관리를 개발자가 아닌 컨테이너에서 직접 관리하는 것을 말한다.
- 스프링은 IoC 구조를 통해 구동 시 필요한 객체의 생성부터 소멸 까지 해당 객체에 대한 관리를 직접 수행한다.
...
- @Reposittory : 웹 애플리케이션에서 영속성(파일, 데이터베이스) 처리 
