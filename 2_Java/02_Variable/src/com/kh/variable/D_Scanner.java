...
public class D_Scanner {
		 ...
		 * 3. Scanner 메서드 사용 
		 * 	  입력값이
		 * 	  - 문자열일 떄 : sc.next()  - 공백이 있을 경우 공백 이전까지
		 * 				   sc.nextLine() - 공백 포함 엔터 전까지 
		 * 	  - 정수일 떄 : sc.nextInt()
		 * 	  - 실수일 떄 : sc.nextFloat(), sc.nextDouble()
		 * 
		 * 	  => 권장하기는 sc.nextLine() 만 사용하는 것을 권장! 애만 enter 처리 
		 * */
...
//			char gender = sc.nextLine().charAt(0); <-- 한줄로 표현 
	        ...
	        System.out.printf("당신의 이름은 %s이고 사는 곳은 %s, 나이는 %d살, 키는 %fcm, 성별은 %c 입니다.", name, addr, age, height, gender);
