...
public class C_Sorting {
	 ...
	 * 
	 * 선택정렬(selection sort)
	 *  - 가장작은 데이터를 선택해 맨 앞에 있는 데이터와 바꾸고,
	 *    그다음 작은 데이터를 선택해 앞에서 두 번쨰 데어터와 바꾸는 과정을 반복 
	 *  - 배열을 전부 탐색하여 최소값을 고르고 왼쪽부터 채워나가는 방식의 정렬
	 *  - 데이터 양이 적을 떄 좋음 : 하지만 100개 이상인 경우 급격한 속도 저하 
	 * 
	 * */
	...
	/*
	 * 삽입정렬(insertion sort) : 동작 원리는 직관적, 선택 정렬에 비해서는 구현 나이도가 높아짐
