 ...
 * - 미완성 클래스이기 때문에 new 연산자를 통해서 객체생성 x
 ...
 * - 추상 클래스를 상속 받는 자식 클래스에서 "반드시" 오버라이딩(재정의) 해야 한다.
 * - 오바라이딩(재정의) 해주지 않으면 컴파일 에러 발생
 * - 자식 클래스에 가제성을 부여하는 방법
 * 
 * */
...
public class Application {
	...
	public static void main(String[] args) {
		...
		// 피자 관련된 출력 과정
		Pizza[] pizza = { new BulgogiPizza(30000, "피자헛"), new PotatoPizza(20000, "도미노피자"),
		...
		/*
		 * 피자헛의 불고기 피자 가격은 3000원 피자 반죽과 함께 도우를 빚다. 토핑은 불고기를 포함시킨다. 피자를 180도에서 10분간 구운다.
		 * 피자를 8등분으로 자른다. 피자를 포장한다.
		 * 
		 * 도미노피자의 포테이토 피자 가격은 2000원 피자반 반죽과 함께 도우를 빚다. 토핑은 감자를 포함시킨다. 피자를 180도에서 10분간
		 * 구운다. 피자를 8등분으로 자른다. 피자를 포장한다.
		 * 
		 * 잭스피자의 파인애플 피자 가격은 10000원 피자 반죽과 함께 도우를 빚다. 토핑은 파인애플을 포함시킨다. 피자를 180도에서 10분간
		 * 구운다. 피자를 8등분으로 자른다. 피자를 포장한다.
		 *
		 * 
		 */
